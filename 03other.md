...
- 系统软件层：不同速度缓冲区匹配、提供系统调用、逻辑块  没有中断、spooling系统
- 顺序：系统设备控制器
- 分类：
  - DMA：块设备主要、不经过CPU
    - 阶段选传送
...
- n个缓冲区错误：mutex0和pp颠倒
- 读者写者：wp12可以取消
- 全选：进程改变的时间 分布式特点 通道功能 进程时机 存储管理范畴 程序在内存中移动 **工作问题条件支持**
- PV不足不选“不能”
- 硬件互斥不选“可以”
- 存储保护选“三不”
